...
fix_type TrackFilter::trackfilter_parse_fix(int* nsats)
{
  ...
  if (!case_ignore_strcmp(static_cast<const char*>(opt_fix), "pps")) {
    *nsats = 4;
    return fix_pps;
  }
  if (!case_ignore_strcmp(static_cast<const char*>(opt_fix), "dgps")) {
    *nsats = 4;
    return fix_dgps;
  }
  if (!case_ignore_strcmp(static_cast<const char*>(opt_fix), "3d")) {
    *nsats = 4;
    return fix_3d;
  }
  if (!case_ignore_strcmp(static_cast<const char*>(opt_fix), "2d")) {
    *nsats = 3;
    return fix_2d;
  }
  if (!case_ignore_strcmp(static_cast<const char*>(opt_fix), "none")) {
    *nsats = 0;
    return fix_none;
  }
...
void TrackFilter::trackfilter_fill_track_list_cb(const route_head* track) 	/* callback for track_disp_all */
{
  ...
  if (opt_name != nullptr) {
    QRegularExpression regex(QRegularExpression::wildcardToRegularExpression(static_cast<const char*>(opt_name)),
...
void TrackFilter::trackfilter_split_init_rte_name(route_head* track, const gpsbabel::DateTime& dt)
{
  ...
  if ((opt_title != nullptr) && (strlen(opt_title) > 0)) {
    if (strchr(opt_title, '%') != nullptr) {
      ...
      track->rte_name = QStringLiteral("%1-%2").arg(static_cast<const char*>(opt_title), datetimestring);
    }
...
void TrackFilter::trackfilter_split()
{
  if (track_list.size() > 1) {
    ...
    if (opt_interval != 0) {
      ...
      assert(re.isValid());
      QRegularExpressionMatch match = re.match(static_cast<const char*>(opt_split));
      if (match.hasMatch()) {
        bool ok;
    ...
    if (opt_distance != 0) {
      ...
      assert(re.isValid());
      QRegularExpressionMatch match = re.match(static_cast<const char*>(opt_sdistance));
      if (match.hasMatch()) {
        bool ok;
