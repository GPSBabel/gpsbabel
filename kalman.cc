...
void Kalman::process() {
    ...
    // Tunable thresholds
    double MAX_REASONABLE_SPEED = max_speed_option_.get_result();   // m/s (~54 km/h, fast cycling)
    ...
    for (RouteList::iterator route_it = global_track_list->begin();
        ...
        if (current_profile == "walking") {
            MAX_REASONABLE_SPEED = 2.0; // e.g., 7.2 km/h
            if (!r_scale_option_.has_value()) r_scale_ = 100.0;
            if (!q_scale_pos_option_.has_value()) q_scale_pos_ = 0.01;
            if (!q_scale_vel_option_.has_value()) q_scale_vel_ = 0.001;
            ...
            if (!interp_max_dt_option_.has_value()) interp_max_dt_ = 120.0; // 2 minutes
            if (!interp_min_multiplier_option_.has_value()) interp_min_multiplier_ = 1.8;
        } else if (current_profile == "cycling") {
            MAX_REASONABLE_SPEED = 15.0; // e.g., 54 km/h
            if (!r_scale_option_.has_value()) r_scale_ = 1.0;
            if (!q_scale_pos_option_.has_value()) q_scale_pos_ = 1.0;
            ...
            MAX_REASONABLE_SPEED = 50.0; // e.g., 180 km/h
            if (!r_scale_option_.has_value()) r_scale_ = 0.1;
            if (!q_scale_pos_option_.has_value()) q_scale_pos_ = 10.0;
            if (!q_scale_vel_option_.has_value()) q_scale_vel_ = 1.0;
            if (!interp_max_dt_option_.has_value()) interp_max_dt_ = 600.0; // 10 minutes
            if (!interp_min_multiplier_option_.has_value()) interp_min_multiplier_ = 1.2;
        } else if (current_profile == "flying") {
            MAX_REASONABLE_SPEED = 300.0; // e.g., 1080 km/h
            if (!r_scale_option_.has_value()) r_scale_ = 0.01;
            if (!q_scale_pos_option_.has_value()) q_scale_pos_ = 100.0;
            if (!q_scale_vel_option_.has_value()) q_scale_vel_ = 10.0;
            if (!interp_max_dt_option_.has_value()) interp_max_dt_ = 3600.0; // 1 hour
            if (!interp_min_multiplier_option_.has_value()) interp_min_multiplier_ = 1.1;
