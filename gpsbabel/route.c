...
route_backup(unsigned int *count, queue **head_bak)
{
	...
	QUEUE_FOR_EACH(qbackup, elem, tmp)
	{
	    ...
	    if (rte_old->an1_extras != NULL)
		rte_old->an1_extras->copy((void **)(&rte_new->an1_extras), (void *)rte_old->an1_extras);
...
track_backup(unsigned int *count, queue **head_bak)
{
	...
	QUEUE_FOR_EACH(qbackup, elem, tmp)
	{
	    ...
	    if (trk_old->an1_extras != NULL)
		trk_old->an1_extras->copy((void **)(&trk_new->an1_extras), (void *)trk_old->an1_extras );
