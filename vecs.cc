...
void Vecs::assign_option(const QString& module, arglist_t* arg, const char* val)
{
  ...
  case ARGTYPE_INT:
    if (*c == '\0') {
      ...
      int test;
      is_fatal(1 != sscanf(c, "%d", &test), "%s: Invalid parameter value %s for option %s", qPrintable(module), val, arg->argstring);
    }
    break;
  case ARGTYPE_FLOAT:
    if (*c == '\0') {
      ...
      double test;
      is_fatal(1 != sscanf(c, "%lf", &test), "%s: Invalid parameter value %s for option %s", qPrintable(module), val, arg->argstring);
    }
    break;
  case ARGTYPE_BOOL:
    if (*c == '\0') {
