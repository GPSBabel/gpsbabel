...
int TpoFormatBase::tpo_read_int()
{
  ...
  case 0xff:  // 32-bit value
    if constexpr(debug) {
      db.gbLog("Found 32-bit value indicator: %x\n", val);
    }
  ...
  case 0xfe:  // 16-bit value
    if constexpr(debug) {
      db.gbLog("Found 16-bit value indicator: %x\n", val);
    }
...
int TpoFormatBase::tpo_find_block(unsigned int block_desired)
{
  ...
  do {
    ...
    if constexpr(debug) {
      db.gbLog("Block: %08x\tat offset: %08x\n", block_type, block_offset);
    }
...
void TpoFormatBase::tpo_process_tracks()
{
  ...
  if constexpr(debug) {
    db.gbLog("Processing Track Styles... (added in 2012 by SRE)\n");
  }
  // Find block 0x050000 (definitions of styles for free-hand routes)
  if (tpo_find_block(0x050000)) {
    if constexpr(debug) {
      db.gbLog("Found no track styles, skipping tracks entirely\n");
    }
    return;
  }
  ...
  if constexpr(debug) {
    db.gbLog("Unpacking %u track styles...\n",track_style_count);
  }
  ...
  for (unsigned ii = 0; ii < track_style_count; ii++) {
    ...
    for (unsigned xx = 0; xx < 2; xx++) {
      ...
      if constexpr(debug > 1) {
        db.gbLog("Skipping unknown byte 0x%x (? per-zoom-level visibility ?)\n", skipped);
      }
    }
    ...
    if constexpr(debug > 2) {
      db.gbLog("Skipping unknown byte 0x%x after color (? always zero ?)\n",tmp);
    }
    ...
    if constexpr(debug > 1) {
      db.gbLog("Track style %u has %d-byte (0x%x) name\n", ii, tmp, tmp);
    }
    ...
    for (unsigned xx = 0; xx < 2; xx++) {
      tmp = gbfgetc(tpo_file_in);
      if constexpr(debug > 2) {
        db.gbLog("Skipping trailing line style byte 0x%x (? always zero ?)\n", tmp);
      }
    }
  ...
  if constexpr(debug) {
    db.gbLog("Done Processing Track Styles... found %u styles\n", track_style_count);
  }

  // Find block 0x060000 (free-hand routes) (original track code, pre-2012, without styles)
  if (tpo_find_block(0x060000)) {
    if constexpr(debug) {
      db.gbLog("Found no track data block, skipping all tracks!\n");
    }
    return;
  }
  ...
  if constexpr(debug) {
    db.gbLog("Number of tracks in file: %u\n", track_count);
  }

  if (track_count == 0) {
    if constexpr(debug) {
      db.gbLog("Found no track data, even though there was a track data block!\n");
    }
    return;
  }
...
#define FullPointTag   0x88
    ...
    for (unsigned int jj = 0; jj < track_byte_count;) { // NO INCREMENT - advance "jj" in the loop
      ...
      if constexpr(debug > 3) {
        db.gbLog("%02x %02x %02x %02x - byte %u, track %u, llvallid=%d\n",
...
//printf("L");
        ...
        if (jj+3<track_byte_count
          ...
          if constexpr(debug > 3) {
            db.gbLog("%02x %02x %02x %02x - raw lon scale = %d (byte %u)\n", buf[jj], buf[jj+1], buf[jj+2], buf[jj+3], lonscale, jj);
          }
//printf(" LONSCALE:");
//printf("%02x%02x%02x%02x", buf[jj], buf[jj+1], buf[jj+2], buf[jj+3]);
        ...
        if (jj+3<track_byte_count
          ...
          if constexpr(debug > 3) {
            db.gbLog("%02x %02x %02x %02x - raw lat scale = %d (byte %u)\n", buf[jj], buf[jj+1], buf[jj+2], buf[jj+3], latscale, jj);
          }
//printf(" LATSCALE:");
//printf("%02x%02x%02x%02x ", buf[jj], buf[jj+1], buf[jj+2], buf[jj+3]);
          jj+=4;
        ...
        cnttp++;
        if constexpr(debug > 3) {
          db.gbLog("Adding BASIC trackpoint #%i: lat=%.5f, lon=%.5f\n", cnttp, waypoint_temp->latitude, waypoint_temp->longitude);
        }
      }
#else
      if constexpr(debug > 3) {
        db.gbLog("%02x %02x %02x %02x = bytes %u-%u (track %u, mode now %s)\n",
      ...
      // offsets.
        ...
        if constexpr(debug > 3) {
          db.gbLog("%02x should mean full lat/lon comes next (byte %u)\n",buf[jj],jj);
        }
        jj++;
        llvalid = 0;
      }
      ...
      // combo embedded in this track next.
        ...
        if constexpr(debug > 3) {
          db.gbLog("%02x should mean full lat/lon or lonscale/latscale comes next (at byte %u)\n",buf[jj],jj);
        }
//printf(" ZERO ");
        jj++;
        llvalid = 0;
      ...
      // Process the delta
        ...
        if constexpr(debug > 3) {
          db.gbLog("%02x - adjusting prev lat/lon from %i/%i", buf[jj], lat, lon);
        }
        lon += lonscale * scarray[buf[jj] >> 4];
        lat += latscale * scarray[(buf[jj] & 0xf)];
        if constexpr(debug > 3) {
          db.gbLog(" to %i/%i, byte %u\n", lat, lon, jj);
        }
//printf(".");
        jj++;
        ...
        cnttp++;
        if constexpr(debug > 3) {
          db.gbLog("Adding ADJUSTED trackpoint #%i: lat=%.5f, lon=%.5f\n", cnttp, waypoint_temp->latitude, waypoint_temp->longitude);
        }
      }
#else
      ...
      if (tpmode == ScaleOneByte) {
        ...
        if (buf[jj] == FullPointTag) {
          if constexpr(debug > 3) {
            db.gbLog("%02x should mean full lat/lon comes next (at byte %u)\n",buf[jj],jj);
          }
          jj++;
          tpmode = GetFullPoint;
          continue; // for jj
