...
gzapi_seek(gbfile* self, int32_t offset, int whence)
{
  ...
  if (result < 0) {
    if (self->is_pipe) {
      fatal("This format cannot be used in piped commands!\n");
    }
    fatal("online compression not yet supported for this format!");
  }
  return 0;
}
...
gzapi_read(void* buf, const gbsize_t size, const gbsize_t members, gbfile* self)
{
  ...
  if ((members == 1) && (size > 1) && (result > 0) && (result < (int)size)) {
    fatal("Unexpected end of file (EOF)!\n");
  }
...
gzapi_ungetc(const int c, gbfile* self)
{
  if (self->back == -1) {
    ...
    fatal("Cannot store more than one byte back!\n");
  }
  return c;
}
...
stdapi_seek(gbfile* self, int32_t offset, int whence)
{
  ...
  if (result != 0) {
    ...
    case SEEK_SET:
      ...
      break;
    default:
      fatal("Unknown seek operation (%d) for file %s!\n",
            whence, qPrintable(self->name));
    }
    fatal("Unable to set file (%s) to position (%llu)!\n",
          qPrintable(self->name), (long long unsigned) pos);
  }
  return 0;
}
...
stdapi_read(void* buf, const gbsize_t size, const gbsize_t members, gbfile* self)
{
  ...
  if ((result < members) && (error_number = ferror(self->handle.std))) {
    fatal("Error %d occurred during read of file '%s'!\n",
          error_number, qPrintable(self->name));
  }
  return result;
}
...
QByteArray gbfreadbuf(gbsize_t size, gbfile* file) {
  ...
  if (nbytes != size) {
    fatal(FatalMsg() << "Attempted to read " << size <<
...
gbfwrite(const void* buf, const gbsize_t size, const gbsize_t members, gbfile* file)
{
  ...
  if (result != members) {
    fatal("Could not write %lld bytes to %s (result %d)!\n",
...
gbftell(gbfile* file)
{
  ...
  if ((signed) result == -1)
    fatal("Could not determine position of file '%s'!\n",
          qPrintable(file->name));
  return result;
}
...
gbfgetutf16char(gbfile* file)
{
    ...
    if (c1 == EOF) {
        fatal("Incomplete unicode (UTF-16%cE) character at EOF!\n",
              file->big_endian ? 'B' : 'L');
    }
  ...
  for (;;) {
    ...
    if (qch == u'\r') {
      ...
      if (qch2 != u'\n') {  // including qch2.isNull()
        ...
        fatal("Invalid unicode (UTF-16%cE) line break!\n",
              file->big_endian ? 'B' : 'L');
      }
      break;
    } else if (qch == u'\n') {
    ...
    if (qch.isLowSurrogate()) {
      fatal("Leading unicode (UTF-16%cE) low surrogate!\n",
            file->big_endian ? 'B' : 'L');
    }
    ...
    if (qch.isHighSurrogate()) {
      ...
      if (!qch2.isLowSurrogate()) { // including qch2.isNull()
        fatal("Missing unicode (UTF-16%cE) low surrogate!\n",
              file->big_endian ? 'B' : 'L');
      }
      str.append(qch2);
    }
