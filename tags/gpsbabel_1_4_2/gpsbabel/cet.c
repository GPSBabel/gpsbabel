...
cet_ucs4_to_utf8(char *dest, size_t dest_size, int value)
{
	...
	{
	    if (dest_size < 1) return (dest_size - 1);
	    *c++ = value;
	    result = 1;
	}
	else if ((value & 0xfffff800) == 0)		/* <= 11 bits */
	{
	    if (dest_size < 2) return (dest_size - 2);
	    *c++ = (0xc0 | (value >> 6));
	    *c++ = (0x80 | (value & 0x3f));
	    result = 2;
	...
	{
	    if (dest_size < 3) return (dest_size - 3);
	    *c++ = (0xe0 |  (value >> 12));
	    *c++ = (0x80 | ((value >> 6) & 0x3f));
	    *c++ = (0x80 |  (value & 0x3f));
	    result = 3;
	...
	{
	    if (dest_size < 4) return (dest_size - 4);
	    *c++ = (0xf0 |  (value >> 18));
	...
	{
	    if (dest_size < 5) return (dest_size - 5);
	    *c++ = (0xf8 |  (value >> 24));
	...
	{
	    if (dest_size < 6) return (dest_size - 6);
	    *c++ = (0xfc |  (value >> 30));
...
cet_ucs4_to_char(const int value, const cet_cs_vec_t *vec)
{
	...
	if ((link = (cet_ucs4_link_t *)vec->ucs4_link))
	{
		...
		if (x < value) i = a + 1;
		else if (x > value) j = a - 1;
		else return link[a].origin;
	    }
	}
